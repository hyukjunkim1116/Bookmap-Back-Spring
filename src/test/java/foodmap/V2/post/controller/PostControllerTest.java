...
class PostControllerTest {
    ...
    void get() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/api/posts/")
                        .content(objectMapper.writeValueAsString(postSearchRequestDTO))
    ...
    void getPostDetail() throws Exception {
        ...
        //when
        // expected
        var result =mockMvc.perform(MockMvcRequestBuilders.get("/api/posts/"+post.getId())
                        .contentType(APPLICATION_JSON))
                .andExpect(status().isOk())
                .andDo(print())
                .andReturn();
        mockMvc.perform(MockMvcRequestBuilders.get("/api/posts/"+post.getId())
                        .contentType(APPLICATION_JSON))
                .andExpect(status().isOk());
        assertEquals("1",Objects.requireNonNull(result.getResponse().getCookie("hit")).getValue());
    }
    @Test
    @BookmapMockUser
    @DisplayName("게시글 수정")
